...
const firstAnswer = "Romulus";
const secondAnswer = "Minato Namikaze";
let point = 0;

check.addEventListener("click",function(){
    const showPoint = document.getElementById("point"); // დავალება 4-ის
    let answers1 = document.querySelectorAll(".firstQuestion");
    answers1.forEach((ans)=>{
        if(ans.checked){
            if(ans.getAttribute("value") === firstAnswer){
    ...
    let answers2 = document.querySelectorAll(".secondQuestion");
    answers2.forEach((ans)=>{
        if(ans.checked){
            if(ans.getAttribute("value") === secondAnswer){
                ans.nextElementSibling.style.color = "green";
                point ++;
            }else{
                 ans.nextElementSibling.style.color = "red";
